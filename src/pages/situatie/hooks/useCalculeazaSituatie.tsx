...
export const useCalculeazaSituatie = ({
  ...
  const groupedPayments = useMemo(() => {
    if (!platiLunare?.length || !persoane?.length) return {}; // Ensure data exists

    return platiLunare.reduce(
      (acc, payment) => {
        if (!payment.paid) return acc; // Skip unpaid payments

        // Find the corresponding price
        const price = monthlyPricesFiltered?.find((x) => x.month_id === payment.month_id)?.price_value ?? 0;

        // Ensure acc[payment.person_id] exists
        if (!acc[payment.person_id]) {
          acc[payment.person_id] = { total: 0, paidMonths: 0 };
        }

        // Update values
        acc[payment.person_id].total += price;
        acc[payment.person_id].paidMonths += price ? 1 : 0;

        return acc;
      },
      Object.fromEntries(persoane.map((p) => [p.id, { total: 0, paidMonths: 0 }]))
    );
  }, [platiLunare, persoane, monthlyPricesFiltered]); // Dependencies ensure updates
